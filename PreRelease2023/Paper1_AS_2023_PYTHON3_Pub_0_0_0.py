...
def EditSourceCode(SourceCode): 
  ...
  while LineNumber > int(SourceCode[0]):
    print("Cannot be greater than ", SourceCode[0])
    LineNumber = int(input("Enter line number of code to edit: "))
...
def Assemble(SourceCode, Memory):
  ...
  if Memory[0].OpCode != "ERR":
    Memory[0].OpCode = "JMP"
    if "       HLT" in SourceCode:
      print("HLT is in SourceCode")
      return Memory
...
def CarryFlag(Memory, Registers, Address):
  Registers[ACC] = Registers[ACC] + Memory[Address].OperandValue
  Registers = SetFlags(Registers[ACC], Registers)
  if Registers > MAX_INT:
    return CARRY
  return Registers

def ORR(Memory, Address, Registers):
  Registers[ACC] = Registers[ACC] | Memory[Address].OperandValue
  Registers = SetFlags(Registers[ACC], Registers)
  return Registers

def AND(Memory, Address, Registers):
  Registers[ACC] = Registers[ACC] & Memory[Address].OperandValue
  Registers = SetFlags(Registers[ACC], Registers)
  return Registers

def EOR(Memory, Address, Registers):
  Registers[ACC] = Registers[ACC] ^ Memory[Address].OperandValue
  Registers = SetFlags(Registers[ACC], Registers)
  return Registers
...
def AssemblerSimulator():
  ...
  while not Finished:
    ...
    if MenuOption == 'L':
      ...
      m = str(input("Enter new file name: "))
      f = open(m, "w")
      for x in SourceCode[1:]:
        f.write(x + "\n")
      f.close()
